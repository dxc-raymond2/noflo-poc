...
describe('ComponentLoader with no external packages installed', function() {
  ...
  it('should not have any packages in the checked list', function() {
    return chai.expect(l.checked).to.not.exist;
  });
  ...
  describe('loading a subgraph', function() {
    ...
    it('should remove `graph` and `start` ports', function(done) {
      return l.listComponents(function(err, components) {
        if (err) {
          return done(err);
        }
        ...
        return l.load('Subgraph', function(err, inst) {
          if (err) {
            return done(err);
          }
          chai.expect(inst).to.be.an('object');
          return inst.once('ready', function() {
    ...
    return it('should also work with a passed graph object', function(done) {
      return noflo.graph.loadFile(file, function(err, graph) {
        if (err) {
          return done(err);
        }
        return l.listComponents(function(err, components) {
  ...
  describe('reading sources', function() {
    ...
    it('should be able to provide source for a graph file component', function(done) {
      ...
      file = urlPrefix + "spec/fixtures/subgraph.fbp";
      l.components.Subgraph = file;
      return l.getSource('Subgraph', function(err, src) {
        if (err) {
          return done(err);
        }
    ...
    return it('should be able to provide source for a graph object component', function(done) {
      var file;
      file = urlPrefix + "spec/fixtures/subgraph.fbp";
      return noflo.graph.loadFile(file, function(err, graph) {
        if (err) {
        ...
        return l.getSource('Subgraph2', function(err, src) {
          if (err) {
            return done(err);
          }
          chai.expect(src.code).to.not.be.empty;
